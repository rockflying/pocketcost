...
//		db.insert("TBL_EXPENDITURE", new String[] {"AMOUNT","DATE"}, new String[] {"100",format(calendar.getTime())});
		...
		if (cursor.moveToNext()) {
			...
			budget_balance_amount_tv.setText("¥ " + (double)budget_balance_amount);
		}
...
//		week_datestr_tv.setText(weekStart + "-" + weekEnd);
		...
		if(budget_amount!=0) Vaule= (double)(expense_amount)/budget_amount;
		
		if(Vaule>=1) progressVaule=100;
		else if(Vaule>=0.01) progressVaule=(int)(100*Vaule);
		else  progressVaule=0;
