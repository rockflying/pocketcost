...
import com.hzm.lczs.utility.MyProcessBar;
...
public class MainActivity extends Activity implements OnClickListener {
	...
	private int budget_amount = 0;
	private int budget_balance_amount = 0;
	private int progressVaule;
	...
	public void onClick(View v) {
		...
		case R.id.today_row_rl:
			showNavExpenseActivity(curtime, curtime,
					getString(R.string.text_title_today),
					NavExpenseActivity.mode_day);
			break;
		case R.id.week_row_rl:
			showNavExpenseActivity(getFirstOfWeek(curtime),
					getLastOfWeek(curtime),
					getString(R.string.text_title_week),
					NavExpenseActivity.mode_week);
			break;
		case R.id.month_row_rl:
			showNavExpenseActivity(getFirstOfMonth(curtime),
					getLastOfMonth(curtime),
					getString(R.string.text_title_month),
					NavExpenseActivity.mode_month);
			break;
	...
	private void showNavExpenseActivity(long startTime, long endTime,
	...
	private void initInfo() {
		...
		db = MyDbHelper.getInstance(MainActivity.this);
		db.open();
		// text
		// calendar.set(2011, 0, 3);
		// db.insert("TBL_EXPENDITURE", new String[] {"AMOUNT","DATE"}, new
		// String[] {"100",format(calendar.getTime())});
		...
		cursor = db.rawQuery(
				"select sum(BUDGET) from TBL_EXPENDITURE_CATEGORY", null);
		if (cursor.moveToNext()) {
			budget_amount = (int) cursor.getDouble(0); // 总预算
			budget_balance_amount = (int) budget_amount - expense_amount;// 预算余额
			budget_balance_amount_tv.setText("¥ "
					+ (double) budget_balance_amount);
		}
		...
		cursor = db
				.rawQuery(
						"select sum(AMOUNT) from TBL_EXPENDITURE where strftime('%Y-%m-%d',DATE)=?",
						new String[] { format(calendar.getTime(), 0) });
		if (cursor.moveToNext()) {
		...
		cursor = db
				.rawQuery(
						"select sum(AMOUNT) from TBL_INCOME where strftime('%Y-%m-%d',DATE)=?",
						new String[] { format(calendar.getTime(), 0) });
		if (cursor.moveToNext()) {
		...
		cursor = db
				.rawQuery(
						"select sum(AMOUNT) from TBL_EXPENDITURE where strftime('%Y-%m-%d',DATE)>=? and strftime('%Y-%m-%d',DATE)<=?",
						new String[] { getMondayOFWeek(), getCurrentWeekday() });
		if (cursor.moveToNext()) {
		...
		cursor = db
				.rawQuery(
						"select sum(AMOUNT) from TBL_INCOME where strftime('%Y-%m-%d',DATE)>=? and strftime('%Y-%m-%d',DATE)<=?",
						new String[] { getMondayOFWeek(), getCurrentWeekday() });
		if (cursor.moveToNext()) {
		...
		month_datestr_tv.setText(month + "月01日-" + month + "月"
				+ getDefaultDay() + "日");

		cursor = db
				.rawQuery(
						"select sum(AMOUNT) from TBL_EXPENDITURE where strftime('%Y%m',DATE)=?",
		...
		cursor = db
				.rawQuery(
						"select sum(AMOUNT) from TBL_INCOME where strftime('%Y%m',DATE)=?",
						new String[] { year + month });
		if (cursor.moveToNext()) {
		...
		if (budget_amount != 0)
			Vaule = (double) (expense_amount) / budget_amount;

		if (Vaule >= 1)
			progressVaule = 100;
		else if (Vaule >= 0.01)
			progressVaule = (int) (100 * Vaule);
		else
			progressVaule = 0;

		myProcessBar.setProgress(progressVaule);// expense_amount/budget_amount

		Log.e("hzm_p", "setProgress:" + progressVaule);
	...
	public void onBackPressed() {
		new AlertDialog.Builder(this)
				...
				.setPositiveButton(getString(R.string.exit_ok),
				...
				.setNegativeButton(getString(R.string.exit_cancle),
						new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface arg0, int arg1) {
								// TODO Auto-generated method stub
