...
public class CommonData {
	...
	/**添加转账信息*/
	public boolean addTransfer(TransferData adata,String name)
	{
		account.values();
		String values[] = new String[]{
		...
		db.open();
		db.insert(MyDbInfo.getTableNames()[11], new String[]{"AMOUNT", "ACCOUNT_ID", "ITEM_ID", "DATE", "MEMO"}, values);
		
		
		AccountData accountData = account.get(adata.amount_id);
		if(accountData==null)  accountData =   getAccountData(name);//course_spinner.getSelectedItem().toString();
...
//		};
		...
		budget_bar_bg = new int[]{
				...
				R.drawable.widget_progress_gre_left,
				R.drawable.widget_progress_gre_middle,
				R.drawable.widget_progress_gre_right,
		};
		db.open();
	   ...
	   Iterator<BudgetData> it =  budgetcategory.values().iterator();
       while(it.hasNext()){          
           BudgetData data = it.next();
           int id=data.id;
           Cursor cur =db.rawQuery("select sum(AMOUNT) from TBL_EXPENDITURE where EXPENDITURE_CATEGORY_ID=?", new String[]{String.valueOf(id)});//
           while (cur.moveToNext()) {
               data.balance_cost =data.balance - cur.getDouble(0);
           } 
           cur.close(); 
       }
